...
class ResNet(nn.Module):
    def __init__(
        ...
        self.rotation_head = nn.Sequential(
            ...
            nn.BatchNorm1d(hidden_mlp),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_mlp, 4),
        )

        self.jigsaw_fc = nn.Sequential(
                nn.Linear(num_out_filters * block.expansion, 256, bias=False),
                nn.BatchNorm1d(256),
                nn.ReLU(inplace=True),
        )
        self.jigsaw_head = nn.Linear(9 * 256, 100)
    ...
    def forward_head(self, x):
        if self.projection_head is not None:
            x_out = self.projection_head(x)

        if self.l2norm:
            x_out = nn.functional.normalize(x_out, dim=1, p=2)

        if self.prototypes is not None:
            return x_out, self.prototypes(x_out)
        return x

    def forward(self, inputs, rot1, rot2):
        ...
        return self.forward_head(output), self.forward_rotation(rot1), self.forward_rotation(rot2)

    def forward_rotation(self, input):
        feature = self.forward_backbone(input.cuda(non_blocking=True))
        return self.rotation_head(feature)

    def forward_jigsaw(self, x):
        x = x.cuda(non_blocking=True)
        x = x.transpose(0, 1)  # B x num_patches x ... --> num_patches x B x ...
        z_list = []
        for i in range(x.shape[0]):
            z = self.forward_backbone(x[i])
            z = z.view(z.shape[0], -1)
            z = self.jigsaw_fc(z)
            z_list.append(z)
        z = torch.cat(z_list, 1)  # B x (num_patches * hidden_dim) x H x W
        return self.jigsaw_head(z)
